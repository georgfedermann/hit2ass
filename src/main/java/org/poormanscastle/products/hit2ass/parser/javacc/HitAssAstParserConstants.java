/* Generated By:JavaCC: Do not edit this line. HitAssAstParserConstants.java */
package org.poormanscastle.products.hit2ass.parser.javacc;


/**
 * Token literal values and constants.
 * Generated by org.javacc.parser.OtherFilesGen#start()
 */
public interface HitAssAstParserConstants {

  /** End of File. */
  int EOF = 0;
  /** RegularExpression Id. */
  int CLOU_OPENER = 1;
  /** RegularExpression Id. */
  int END_COMMENT = 7;
  /** RegularExpression Id. */
  int IF = 9;
  /** RegularExpression Id. */
  int THEN = 10;
  /** RegularExpression Id. */
  int ELSE = 11;
  /** RegularExpression Id. */
  int ENDIF = 12;
  /** RegularExpression Id. */
  int SB = 13;
  /** RegularExpression Id. */
  int WS = 14;
  /** RegularExpression Id. */
  int FS = 15;
  /** RegularExpression Id. */
  int SS = 16;
  /** RegularExpression Id. */
  int CA = 17;
  /** RegularExpression Id. */
  int MS = 18;
  /** RegularExpression Id. */
  int HC = 19;
  /** RegularExpression Id. */
  int GS = 20;
  /** RegularExpression Id. */
  int JS = 21;
  /** RegularExpression Id. */
  int ZS = 22;
  /** RegularExpression Id. */
  int GD = 23;
  /** RegularExpression Id. */
  int LD = 24;
  /** RegularExpression Id. */
  int LIST = 25;
  /** RegularExpression Id. */
  int DUMP = 26;
  /** RegularExpression Id. */
  int AS = 27;
  /** RegularExpression Id. */
  int IB = 28;
  /** RegularExpression Id. */
  int OF = 29;
  /** RegularExpression Id. */
  int DV = 30;
  /** RegularExpression Id. */
  int PS = 31;
  /** RegularExpression Id. */
  int SK = 32;
  /** RegularExpression Id. */
  int FIXED_TEXT = 33;
  /** RegularExpression Id. */
  int DV_ID = 38;
  /** RegularExpression Id. */
  int FILE_OPEN = 43;
  /** RegularExpression Id. */
  int FILE_CLOSE = 44;
  /** RegularExpression Id. */
  int FILE_READ = 45;
  /** RegularExpression Id. */
  int FILE_TEXT = 46;
  /** RegularExpression Id. */
  int IB_TEXT = 51;
  /** RegularExpression Id. */
  int PS_SB = 60;
  /** RegularExpression Id. */
  int PS_EQ = 61;
  /** RegularExpression Id. */
  int PS_EQ_STRICT = 62;
  /** RegularExpression Id. */
  int PS_OR = 63;
  /** RegularExpression Id. */
  int PS_AND = 64;
  /** RegularExpression Id. */
  int PS_XOR = 65;
  /** RegularExpression Id. */
  int PS_NOT = 66;
  /** RegularExpression Id. */
  int PS_NEQ = 67;
  /** RegularExpression Id. */
  int PS_LT = 68;
  /** RegularExpression Id. */
  int PS_LTE = 69;
  /** RegularExpression Id. */
  int PS_GT = 70;
  /** RegularExpression Id. */
  int PS_GTE = 71;
  /** RegularExpression Id. */
  int PS_PLUS = 72;
  /** RegularExpression Id. */
  int PS_MINUS = 73;
  /** RegularExpression Id. */
  int PS_TIMES = 74;
  /** RegularExpression Id. */
  int PS_DIV = 75;
  /** RegularExpression Id. */
  int PS_COMMA = 76;
  /** RegularExpression Id. */
  int PS_QUOTE = 77;
  /** RegularExpression Id. */
  int PS_LPAREN = 78;
  /** RegularExpression Id. */
  int PS_RPAREN = 79;
  /** RegularExpression Id. */
  int PS_NUM = 80;
  /** RegularExpression Id. */
  int PS_DEC = 81;
  /** RegularExpression Id. */
  int PS_ID = 82;
  /** RegularExpression Id. */
  int PS_TEXT = 83;
  /** RegularExpression Id. */
  int PS_EL_SB = 88;
  /** RegularExpression Id. */
  int PS_EL_EQ = 89;
  /** RegularExpression Id. */
  int PS_EL_EQ_STRICT = 90;
  /** RegularExpression Id. */
  int PS_EL_OR = 91;
  /** RegularExpression Id. */
  int PS_EL_AND = 92;
  /** RegularExpression Id. */
  int PS_EL_XOR = 93;
  /** RegularExpression Id. */
  int PS_EL_NOT = 94;
  /** RegularExpression Id. */
  int PS_EL_NEQ = 95;
  /** RegularExpression Id. */
  int PS_EL_LT = 96;
  /** RegularExpression Id. */
  int PS_EL_LTE = 97;
  /** RegularExpression Id. */
  int PS_EL_GT = 98;
  /** RegularExpression Id. */
  int PS_EL_GTE = 99;
  /** RegularExpression Id. */
  int PS_EL_PLUS = 100;
  /** RegularExpression Id. */
  int PS_EL_MINUS = 101;
  /** RegularExpression Id. */
  int PS_EL_TIMES = 102;
  /** RegularExpression Id. */
  int PS_EL_DIV = 103;
  /** RegularExpression Id. */
  int PS_EL_COMMA = 104;
  /** RegularExpression Id. */
  int PS_EL_QUOTE = 105;
  /** RegularExpression Id. */
  int PS_EL_LPAREN = 106;
  /** RegularExpression Id. */
  int PS_EL_RPAREN = 107;
  /** RegularExpression Id. */
  int PS_EL_NUM = 108;
  /** RegularExpression Id. */
  int PS_EL_DEC = 109;
  /** RegularExpression Id. */
  int PS_EL_ID = 110;
  /** RegularExpression Id. */
  int PS_EL_TEXT = 111;
  /** RegularExpression Id. */
  int PS_IDX_CHECK_LBRACKET = 112;
  /** RegularExpression Id. */
  int PS_IDX_CHECK_LPAREN = 113;
  /** RegularExpression Id. */
  int PS_IDX_CHECK_FIXED_TEXT = 114;
  /** RegularExpression Id. */
  int PS_IDX_SB = 115;
  /** RegularExpression Id. */
  int PS_IDX_CHECK_SKIP = 116;
  /** RegularExpression Id. */
  int PS_IDX_LBRACKET = 121;
  /** RegularExpression Id. */
  int PS_IDX_RBRACKET = 122;
  /** RegularExpression Id. */
  int PS_IDX_EQ = 123;
  /** RegularExpression Id. */
  int PS_IDX_EQ_STRICT = 124;
  /** RegularExpression Id. */
  int PS_IDX_OR = 125;
  /** RegularExpression Id. */
  int PS_IDX_AND = 126;
  /** RegularExpression Id. */
  int PS_IDX_XOR = 127;
  /** RegularExpression Id. */
  int PS_IDX_NOT = 128;
  /** RegularExpression Id. */
  int PS_IDX_NEQ = 129;
  /** RegularExpression Id. */
  int PS_IDX_LT = 130;
  /** RegularExpression Id. */
  int PS_IDX_LTE = 131;
  /** RegularExpression Id. */
  int PS_IDX_GT = 132;
  /** RegularExpression Id. */
  int PS_IDX_GTE = 133;
  /** RegularExpression Id. */
  int PS_IDX_PLUS = 134;
  /** RegularExpression Id. */
  int PS_IDX_MINUS = 135;
  /** RegularExpression Id. */
  int PS_IDX_TIMES = 136;
  /** RegularExpression Id. */
  int PS_IDX_DIV = 137;
  /** RegularExpression Id. */
  int PS_IDX_QUOTE = 138;
  /** RegularExpression Id. */
  int PS_IDX_LPAREN = 139;
  /** RegularExpression Id. */
  int PS_IDX_RPAREN = 140;
  /** RegularExpression Id. */
  int PS_IDX_COMMA = 141;
  /** RegularExpression Id. */
  int PS_IDX_NUM = 142;
  /** RegularExpression Id. */
  int PS_IDX_DEC = 143;
  /** RegularExpression Id. */
  int PS_IDX_ID = 144;
  /** RegularExpression Id. */
  int SS_LPAREN = 149;
  /** RegularExpression Id. */
  int SS_RPAREN = 150;
  /** RegularExpression Id. */
  int SS_QUOTE = 151;
  /** RegularExpression Id. */
  int SS_COMMA = 152;
  /** RegularExpression Id. */
  int SS_LBRACKET = 153;
  /** RegularExpression Id. */
  int SS_RBRACKET = 154;
  /** RegularExpression Id. */
  int SS_NUM = 155;
  /** RegularExpression Id. */
  int SS_ID = 156;
  /** RegularExpression Id. */
  int CA_COLON = 157;
  /** RegularExpression Id. */
  int SK_VAR_ID = 158;
  /** RegularExpression Id. */
  int MS_EQ = 167;
  /** RegularExpression Id. */
  int MS_EQ_STRICT = 168;
  /** RegularExpression Id. */
  int MS_OR = 169;
  /** RegularExpression Id. */
  int MS_AND = 170;
  /** RegularExpression Id. */
  int MS_XOR = 171;
  /** RegularExpression Id. */
  int MS_NOT = 172;
  /** RegularExpression Id. */
  int MS_NEQ = 173;
  /** RegularExpression Id. */
  int MS_LT = 174;
  /** RegularExpression Id. */
  int MS_LTE = 175;
  /** RegularExpression Id. */
  int MS_GT = 176;
  /** RegularExpression Id. */
  int MS_GTE = 177;
  /** RegularExpression Id. */
  int MS_PLUS = 178;
  /** RegularExpression Id. */
  int MS_MINUS = 179;
  /** RegularExpression Id. */
  int MS_TIMES = 180;
  /** RegularExpression Id. */
  int MS_DIV = 181;
  /** RegularExpression Id. */
  int MS_LPAREN = 182;
  /** RegularExpression Id. */
  int MS_RPAREN = 183;
  /** RegularExpression Id. */
  int MS_LBRACKET = 184;
  /** RegularExpression Id. */
  int MS_RBRACKET = 185;
  /** RegularExpression Id. */
  int MS_COMMA = 186;
  /** RegularExpression Id. */
  int MS_NUM = 187;
  /** RegularExpression Id. */
  int MS_DEC = 188;
  /** RegularExpression Id. */
  int MS_ID = 189;
  /** RegularExpression Id. */
  int MS_TEXT = 190;
  /** RegularExpression Id. */
  int IF_EQ = 191;
  /** RegularExpression Id. */
  int IF_EQ_STRICT = 192;
  /** RegularExpression Id. */
  int IF_OR = 193;
  /** RegularExpression Id. */
  int IF_AND = 194;
  /** RegularExpression Id. */
  int IF_XOR = 195;
  /** RegularExpression Id. */
  int IF_NOT = 196;
  /** RegularExpression Id. */
  int IF_NEQ = 197;
  /** RegularExpression Id. */
  int IF_LT = 198;
  /** RegularExpression Id. */
  int IF_LTE = 199;
  /** RegularExpression Id. */
  int IF_GT = 200;
  /** RegularExpression Id. */
  int IF_GTE = 201;
  /** RegularExpression Id. */
  int IF_PLUS = 202;
  /** RegularExpression Id. */
  int IF_MINUS = 203;
  /** RegularExpression Id. */
  int IF_TIMES = 204;
  /** RegularExpression Id. */
  int IF_DIV = 205;
  /** RegularExpression Id. */
  int IF_QUOTE = 206;
  /** RegularExpression Id. */
  int IF_COLON = 207;
  /** RegularExpression Id. */
  int IF_LPAREN = 208;
  /** RegularExpression Id. */
  int IF_RPAREN = 209;
  /** RegularExpression Id. */
  int IF_NUM = 210;
  /** RegularExpression Id. */
  int IF_LBRACKET = 211;
  /** RegularExpression Id. */
  int IF_RBRACKET = 212;
  /** RegularExpression Id. */
  int IF_DEC = 213;
  /** RegularExpression Id. */
  int IF_ID = 214;
  /** RegularExpression Id. */
  int IF_TEXT = 215;
  /** RegularExpression Id. */
  int FOR_EQ = 220;
  /** RegularExpression Id. */
  int FOR_EQ_STRICT = 221;
  /** RegularExpression Id. */
  int FOR_OR = 222;
  /** RegularExpression Id. */
  int FOR_AND = 223;
  /** RegularExpression Id. */
  int FOR_XOR = 224;
  /** RegularExpression Id. */
  int FOR_NOT = 225;
  /** RegularExpression Id. */
  int FOR_NEQ = 226;
  /** RegularExpression Id. */
  int FOR_LT = 227;
  /** RegularExpression Id. */
  int FOR_LTE = 228;
  /** RegularExpression Id. */
  int FOR_GT = 229;
  /** RegularExpression Id. */
  int FOR_GTE = 230;
  /** RegularExpression Id. */
  int FOR_PLUS = 231;
  /** RegularExpression Id. */
  int FOR_MINUS = 232;
  /** RegularExpression Id. */
  int FOR_TIMES = 233;
  /** RegularExpression Id. */
  int FOR_DIV = 234;
  /** RegularExpression Id. */
  int FOR_QUOTE = 235;
  /** RegularExpression Id. */
  int FOR_COLON = 236;
  /** RegularExpression Id. */
  int FOR_LPAREN = 237;
  /** RegularExpression Id. */
  int FOR_RPAREN = 238;
  /** RegularExpression Id. */
  int FOR_NUM = 239;
  /** RegularExpression Id. */
  int FOR_LBRACKET = 240;
  /** RegularExpression Id. */
  int FOR_RBRACKET = 241;
  /** RegularExpression Id. */
  int FOR_DEC = 242;
  /** RegularExpression Id. */
  int FOR_ID = 243;
  /** RegularExpression Id. */
  int FOR_TEXT = 244;
  /** RegularExpression Id. */
  int DS_EQ = 249;
  /** RegularExpression Id. */
  int DS_EQ_STRICT = 250;
  /** RegularExpression Id. */
  int DS_OR = 251;
  /** RegularExpression Id. */
  int DS_AND = 252;
  /** RegularExpression Id. */
  int DS_XOR = 253;
  /** RegularExpression Id. */
  int DS_NOT = 254;
  /** RegularExpression Id. */
  int DS_NEQ = 255;
  /** RegularExpression Id. */
  int DS_LT = 256;
  /** RegularExpression Id. */
  int DS_LTE = 257;
  /** RegularExpression Id. */
  int DS_GT = 258;
  /** RegularExpression Id. */
  int DS_GTE = 259;
  /** RegularExpression Id. */
  int DS_STRING_CONCAT = 260;
  /** RegularExpression Id. */
  int DS_PLUS = 261;
  /** RegularExpression Id. */
  int DS_MINUS = 262;
  /** RegularExpression Id. */
  int DS_TIMES = 263;
  /** RegularExpression Id. */
  int DS_DIV = 264;
  /** RegularExpression Id. */
  int DS_QUOTE = 265;
  /** RegularExpression Id. */
  int DS_COLON = 266;
  /** RegularExpression Id. */
  int DS_COMMA = 267;
  /** RegularExpression Id. */
  int DS_LPAREN = 268;
  /** RegularExpression Id. */
  int DS_RPAREN = 269;
  /** RegularExpression Id. */
  int DS_LBRACKET = 270;
  /** RegularExpression Id. */
  int DS_RBRACKET = 271;
  /** RegularExpression Id. */
  int DS_NUM = 272;
  /** RegularExpression Id. */
  int DS_DEC = 273;
  /** RegularExpression Id. */
  int DS_ID = 274;
  /** RegularExpression Id. */
  int DS_TEXT = 275;
  /** RegularExpression Id. */
  int AS_SB = 280;
  /** RegularExpression Id. */
  int AS_EQ = 281;
  /** RegularExpression Id. */
  int AS_EQ_STRICT = 282;
  /** RegularExpression Id. */
  int AS_OR = 283;
  /** RegularExpression Id. */
  int AS_AND = 284;
  /** RegularExpression Id. */
  int AS_XOR = 285;
  /** RegularExpression Id. */
  int AS_NOT = 286;
  /** RegularExpression Id. */
  int AS_NEQ = 287;
  /** RegularExpression Id. */
  int AS_LT = 288;
  /** RegularExpression Id. */
  int AS_LTE = 289;
  /** RegularExpression Id. */
  int AS_GT = 290;
  /** RegularExpression Id. */
  int AS_GTE = 291;
  /** RegularExpression Id. */
  int AS_COMMA = 292;
  /** RegularExpression Id. */
  int AS_STRING_CONCAT = 293;
  /** RegularExpression Id. */
  int AS_PLUS = 294;
  /** RegularExpression Id. */
  int AS_MINUS = 295;
  /** RegularExpression Id. */
  int AS_TIMES = 296;
  /** RegularExpression Id. */
  int AS_DIV = 297;
  /** RegularExpression Id. */
  int AS_QUOTE = 298;
  /** RegularExpression Id. */
  int AS_COLON = 299;
  /** RegularExpression Id. */
  int AS_LPAREN = 300;
  /** RegularExpression Id. */
  int AS_RPAREN = 301;
  /** RegularExpression Id. */
  int AS_LBRACKET = 302;
  /** RegularExpression Id. */
  int AS_RBRACKET = 303;
  /** RegularExpression Id. */
  int AS_NUM = 304;
  /** RegularExpression Id. */
  int AS_DEC = 305;
  /** RegularExpression Id. */
  int AS_ID = 306;
  /** RegularExpression Id. */
  int AS_TEXT = 307;
  /** RegularExpression Id. */
  int LS_AMPERSAND = 312;
  /** RegularExpression Id. */
  int LS_LBRACE = 313;
  /** RegularExpression Id. */
  int LS_GD = 314;
  /** RegularExpression Id. */
  int LS_LD = 315;
  /** RegularExpression Id. */
  int LS_LBRACKET = 316;
  /** RegularExpression Id. */
  int LS_RBRACKET = 317;
  /** RegularExpression Id. */
  int LS_ID = 318;
  /** RegularExpression Id. */
  int LS_TEXT = 319;
  /** RegularExpression Id. */
  int DUMP_TEXT = 328;
  /** RegularExpression Id. */
  int LS_BRACES_AMPERSAND = 329;
  /** RegularExpression Id. */
  int LS_BRACES_COMMA = 330;
  /** RegularExpression Id. */
  int LS_BRACES_RBRACE = 331;
  /** RegularExpression Id. */
  int LS_BRACES_LBRACKET = 332;
  /** RegularExpression Id. */
  int LS_BRACES_RBRACKET = 333;
  /** RegularExpression Id. */
  int LS_BRACES_NUM = 334;
  /** RegularExpression Id. */
  int LS_BRACES_DEC = 335;
  /** RegularExpression Id. */
  int LS_BRACES_ID = 336;
  /** RegularExpression Id. */
  int LS_BRACES_TEXT = 337;
  /** RegularExpression Id. */
  int HC_HC = 342;
  /** RegularExpression Id. */
  int HC_NUM = 343;
  /** RegularExpression Id. */
  int HC_LBRACKET = 344;
  /** RegularExpression Id. */
  int HC_RBRACKET = 345;
  /** RegularExpression Id. */
  int HC_ID = 346;
  /** RegularExpression Id. */
  int HC_NAME = 347;

  /** Lexical state. */
  int DEFAULT = 0;
  /** Lexical state. */
  int TXT_MODE = 1;
  /** Lexical state. */
  int COMMAND_MODE_DS = 2;
  /** Lexical state. */
  int COMMAND_MODE_SS = 3;
  /** Lexical state. */
  int COMMAND_MODE_AS = 4;
  /** Lexical state. */
  int COMMAND_MODE_MS = 5;
  /** Lexical state. */
  int COMMENT_MODE = 6;
  /** Lexical state. */
  int COMMAND_MODE_DV = 7;
  /** Lexical state. */
  int COMMAND_MODE_FILE = 8;
  /** Lexical state. */
  int COMMAND_MODE_IB = 9;
  /** Lexical state. */
  int COMMAND_MODE_PS = 10;
  /** Lexical state. */
  int COMMAND_MODE_PS_EL = 11;
  /** Lexical state. */
  int COMMAND_MODE_PS_IDX_CHECK = 12;
  /** Lexical state. */
  int COMMAND_MODE_PS_IDX = 13;
  /** Lexical state. */
  int COMMAND_MODE_SK = 14;
  /** Lexical state. */
  int COMMAND_MODE_IF = 15;
  /** Lexical state. */
  int COMMAND_MODE_FOR = 16;
  /** Lexical state. */
  int COMMAND_MODE_LS = 17;
  /** Lexical state. */
  int COMMAND_MODE_DUMP = 18;
  /** Lexical state. */
  int COMMAND_MODE_LS_BRACES = 19;
  /** Lexical state. */
  int COMMAND_MODE_HC = 20;

  /** Literal token values. */
  String[] tokenImage = {
    "<EOF>",
    "<CLOU_OPENER>",
    "\"#*\"",
    "\"#*\"",
    "\"#*\"",
    "\"#*\"",
    "\"#*\"",
    "\"#\"",
    "<token of kind 8>",
    "\"#?\"",
    "\"/J\"",
    "\"/N\"",
    "\"#\"",
    "\"@\"",
    "\"#S\"",
    "\"#W\"",
    "\"#C\"",
    "<CA>",
    "\"#$\"",
    "\"#^\"",
    "\"#G\"",
    "\"#J\"",
    "\"#Z\"",
    "\"#D\"",
    "\"#d\"",
    "\"#L\"",
    "\"#q\"",
    "\"#=\"",
    "\"#B\"",
    "\"#X\"",
    "<DV>",
    "\"#>\"",
    "\"#!\"",
    "<FIXED_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "<DV_ID>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"O\"",
    "\"C\"",
    "\"\\\"r\\\"\"",
    "<FILE_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "<IB_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"@\"",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\",\"",
    "\"\\\"\"",
    "\"(\"",
    "\")\"",
    "<PS_NUM>",
    "<PS_DEC>",
    "<PS_ID>",
    "<PS_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"@\"",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\",\"",
    "\"\\\"\"",
    "\"(\"",
    "\")\"",
    "<PS_EL_NUM>",
    "<PS_EL_DEC>",
    "<PS_EL_ID>",
    "<PS_EL_TEXT>",
    "\"[\"",
    "\"(\"",
    "<PS_IDX_CHECK_FIXED_TEXT>",
    "\"@\"",
    "<PS_IDX_CHECK_SKIP>",
    "\"\\n\"",
    "\"\\t\"",
    "\"\\r\"",
    "\" \"",
    "\"[\"",
    "\"]\"",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"\\\"\"",
    "\"(\"",
    "\")\"",
    "\",\"",
    "<PS_IDX_NUM>",
    "<PS_IDX_DEC>",
    "<PS_IDX_ID>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"(\"",
    "\")\"",
    "\"\\\"\"",
    "\",\"",
    "\"[\"",
    "\"]\"",
    "<SS_NUM>",
    "<SS_ID>",
    "\":\"",
    "<SK_VAR_ID>",
    "\" \"",
    "\"\\n\"",
    "\"\\t\"",
    "\"\\r\"",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"(\"",
    "\")\"",
    "\"[\"",
    "\"]\"",
    "\",\"",
    "<MS_NUM>",
    "<MS_DEC>",
    "<MS_ID>",
    "<MS_TEXT>",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"\\\"\"",
    "\":\"",
    "\"(\"",
    "\")\"",
    "<IF_NUM>",
    "\"[\"",
    "\"]\"",
    "<IF_DEC>",
    "<IF_ID>",
    "<IF_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"\\\"\"",
    "\":\"",
    "\"(\"",
    "\")\"",
    "<FOR_NUM>",
    "\"[\"",
    "\"]\"",
    "<FOR_DEC>",
    "<FOR_ID>",
    "<FOR_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\"&\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"\\\"\"",
    "\":\"",
    "\",\"",
    "\"(\"",
    "\")\"",
    "\"[\"",
    "\"]\"",
    "<DS_NUM>",
    "<DS_DEC>",
    "<DS_ID>",
    "<DS_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"@\"",
    "\"=\"",
    "\"==\"",
    "\"oder\"",
    "\"und\"",
    "\"xor_tbd\"",
    "\"not_tbd\"",
    "\"<>\"",
    "\"<\"",
    "\"<=\"",
    "\">\"",
    "\">=\"",
    "\",\"",
    "\"&\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"\\\"\"",
    "\":\"",
    "\"(\"",
    "\")\"",
    "\"[\"",
    "\"]\"",
    "<AS_NUM>",
    "<AS_DEC>",
    "<AS_ID>",
    "<AS_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"&\"",
    "\"{\"",
    "\"D\"",
    "\"d\"",
    "\"[\"",
    "\"]\"",
    "<LS_ID>",
    "<LS_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"\\n\"",
    "\"\\r\"",
    "\" \"",
    "\"\\t\"",
    "<DUMP_TEXT>",
    "\"&\"",
    "\",\"",
    "\"}\"",
    "\"[\"",
    "\"]\"",
    "<LS_BRACES_NUM>",
    "<LS_BRACES_DEC>",
    "<LS_BRACES_ID>",
    "<LS_BRACES_TEXT>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
    "\"#^\"",
    "<HC_NUM>",
    "\"[\"",
    "\"]\"",
    "<HC_ID>",
    "<HC_NAME>",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\t\"",
    "\" \"",
  };

}
