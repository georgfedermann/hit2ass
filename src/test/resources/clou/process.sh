...
if [ "$1" != "" ]; then
  ...
  cat "$1".clou | hitAssTools.sh -a | dot -Tsvg -Gcharset=ISO-8859-1 | xmllint --format - > "$1".svg && cat "$1".clou | hitAssTools.sh -w | xmllint --format - > "$1".acr && cp --recursive . /mnt/hgfs/shared/hitass/sampleDocuments/clou/ && cp "$1".* /mnt/hgfs/shared/hitass/sampleDocuments/clou/ 
